...
> 2026-07-20 在 dev-a12（新加坡，单 hub，1 vCPU）上实测；2026-07-29 补充双账号扩容验证（§2.6）。
> 一句话：**单账号 ≈41.6K TPM / ≈5 RPS，24 并发是硬边界；超过就撞 Copilot 429，
> 触发 60 秒熔断后实际吞吐反而暴跌。加账号可线性扩容（2 账号 = 2.1×）。**
...
> **多账号线性扩容（已实测）**：加到 2 个 GitHub 账号后 → **48 并发 / ≈88K TPM /
> ≈9.8 RPS**，恰好是单账号的 **2.1 倍**；并且超限时不再整池 503（可 failover）。
> 详见 §2.6。
...
3. **加 GitHub 账号** —— 多 hub 并行，唯一能线性扩展的路径（**已实测 2.1×**，见 §2.6）
...
可 failover），本地调参/加 CPU 都突破不了。**双账号实测已验证**：48 并发下 7 个 429
但 **0 个 503** —— failover 生效，池始终可用（§2.6）。
...
- **扩容唯一有效路径：加 GitHub 账号 —— 已实测线性**：2 个账号 = 48 并发 /
  ≈88K TPM / ≈9.8 RPS（单账号的 2.1×）。多 hub 既分摊负载，又让熔断有地方 failover，
  消除"一次超限 → 60 秒全挂"的单点（48 并发下 0 个 503）。见 §2.6。
- 生产容量规划时按 **≤24 并发/账号** 估算（N 个账号 ≈ 24N 并发），并留出余量避免触发熔断。
