...
| 指标 | 值 | 条件 |
| --- | --- | --- |
| **最大稳定 TPM** | **≈ 41,600 tokens/min** | 24 并发，`max_tokens=1200`，零 429 |
| **最大 RPS** | **≈ 5 req/s** | 受单请求 ≈4s 延迟限制 |
| **并发硬边界** | **24** | 32 起开始撞 429 |
| 单请求延迟 | p50 ≈ 3.5–4.4s，p95 ≈ 5–6.6s | 其中约 3.5s 是固定开销 |

**安全运行区**：≤24 并发。超了不是"慢一点"，是**吞吐崩塌**（见 §3）。
...
### 2.3 GPT vs Claude（2 vCPU，24/48 并发）

| 模型 | 24并发 RPS | 24并发 TPM | 48并发 429 |
| --- | --- | --- | --- |
| gpt-4o-mini | 5.24 | 23,591 | 17 |
| claude-haiku-4.5 | 4.09 | 18,423 | 14 |

→ **两个模型都在 48 并发触顶，行为一致** → 配额是**账号级共享**，不是按模型分配。
...
# 找 TPM 上限（本文的最优点）
python tests/manual/load_test_ramp.py --levels 24 --per-level 48 \
...
TF_MODEL=claude-haiku-4.5 python tests/manual/load_test_ramp.py
```
...
2. **`--per-level` 至少是最大并发的 2–3 倍**。12 个请求在 24 并发下线程池都没跑满，
   实测运行间波动可达 ±20%；48 起才稳。
...
- **单个 GitHub 账号 ≈ 41.6K TPM / 5 RPS / 24 并发**，这是 Copilot 订阅的配额，
